...
def processing_delay_slots(code_rate, coding_depth):
    """computing ptocessing delay in slotss ."""
    k = code_rate[0]
    total = code_rate[1]
    n = total - k
    w = coding_depth * k
...
def objective_function(config, seed, extra_input, num_iterations):
    ...
    return cost if np.isfinite(cost) and cost > 0 else EPS_COST
...
def main():
    ...
    # xrisi alou fakelou gia logs
    seed_folder = os.path.join("smac_logs", f"seed_{args.seed}")
    scenario = Scenario(cs, n_trials=args.n_calls, seed=args.seed, output_directory=seed_folder)
    
    init_design = RandomInitialDesign(scenario, n_configs=max(1, int(args.random_percentage * args.n_calls)))
    target_func = lambda config, seed: objective_function(config, seed, params, args.iterations)

    if args.estimator == "Random": 
        smac = RSfacade(scenario, target_func)
    elif args.estimator == "RF": 
        smac = RFfacade(scenario=scenario, target_function=target_func, initial_design=init_design)
    else: 
        smac = GPfacade(scenario=scenario, target_function=target_func, initial_design=init_design)
    ...
    best_rate_idx = incumbent["rate_index"]
    best_r = StateVectorUtils.valid_rates[best_rate_idx]
    best_d_idx_name = f"d_{best_rate_idx}"
    best_d = StateVectorUtils.calculate_possible_d_values(best_r[0])[incumbent[best_d_idx_name]]
    
    if (best_r, best_d) in reward_terms:
        best_metrics = reward_terms[(best_r, best_d)]
    else:
        print(f" Warning: Incumbent {best_r, best_d} was predicted but not evaluated. Running final simulation...")
        final_run = run_simulation(best_r, best_d, params + [args.seed])
        best_metrics = (final_run[1], final_run[2]) 
    
    best_reward_g = MAX_REWARD - smac.validate(incumbent)
    
    res_path = "examples/bo/results/soo"
    os.makedirs(res_path, exist_ok=True)
    res_file = os.path.join(res_path, "smac_summary.tsv")
    
    header = "Estimator\tN_calls\titerations\tploss\tnum_packets\tseed\tD\tDRT\tBest R\tBest D\tBest Score g\tAvg delivery ratio\tavg inorder delay\truntime (sec)\n"
    row = f"{args.estimator}\t{args.n_calls}\t{args.iterations}\t{args.ploss}\t{args.num_packets}\t{args.seed}\t{args.D}\t{args.DRT}\t{best_r[0]}/{best_r[1]}\t{best_d}\t{best_reward_g:.6f}\t{best_metrics[0]:.6f}\t{best_metrics[1]:.6f}\t{runtime:.2f}\n"

    write_header = not os.path.exists(res_file) or os.path.getsize(res_file) == 0
    with open(res_file, "a") as f:
        if write_header: f.write(header)
        f.write(row)
        
    print(f"\nFinished! Best Config: R={best_r}, d={best_d}, Reward={best_reward_g:.6f}")
