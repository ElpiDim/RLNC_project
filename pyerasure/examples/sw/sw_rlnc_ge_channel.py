...
from pyerasure import GilbertElliotChannel

# Custom script to collect performance data for Gilbert Elliott channels.
...
def main():
    ...
    parser.add_argument("--burst_duration", default = 4, required = False, help = "Average burst duration during the Bad state (in packets)")
    parser.add_argument("--pe_g", default = 0, required = False, help = "Packet erasure probability in Good state")
    parser.add_argument("--pe_b", default = 1, required = False, help = "Packet erasure probability in Bad state")
    ...
    burst_duration = float(args.burst_duration)

    # GE channel parameters
    pe_g = float(args.pe_g)
    pe_b = float(args.pe_b)
    prob_b2g = 1 / burst_duration
    prob_g2b = (prob_b2g*abs(pe_g - ploss)) / abs(ploss - pe_b)
    ...
    print(f"Number of Transmitted Packets: \t{args.num_packets}\n"
          ...
          f"Burst Duration: \t\t{burst_duration}\n"
          f"Seed: \t\t\t\t{args.seed}")
    ...
    for index in range(packets):
        ...
        if not channel.occurred_GE_erasure(): 
