...
def processing_delay_slots(code_rate, coding_depth):
    """
    Baseline processing delay model:
    f(w) = 0.0501 w^2 - 0.0926 w + 17.652
    scaled for arbitrary code rate.
    """
...
def run_simulation(r, d, extraparams):
    ...
    goodput = R * delivery_ratio
    goodput_threshold = 0.7

    if total_delay <= D and goodput >= goodput_threshold:
        reward_score = ((D - total_delay) / D) * goodput
    else:
        reward_score = (
            goodput - goodput_threshold - np.log(1.0 + max(0.0, (total_delay / D) - 1.0))
        )
